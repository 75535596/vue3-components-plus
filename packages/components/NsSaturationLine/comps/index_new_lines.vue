      ...
      >
          ...
          <v-layer ref="layerPipeline">
              <v-group v-for="(item) in pipeLines" :key="item?.pointCode" :config="pipeLineGroupConfig(item)" :ref="el => setPipelineGroupRef(el, item.pointCode)">
                  ...
                  <v-rect :config="pipeLineBkConfig(item)" />
                  <v-text :config="labelTextConfig(item)" />
...
<script setup lang="ts" name="NsSaturationLine">
...
async function initChart() {
  ...
  // 8. 其他图层
  {
      if(data.value.cswlValList?.length) {
        ...
        // otherLinesConfig.value.sceneFunc = (context, shape) => {
        //     context.beginPath();
        //     // 将waterLine1转换为点对象数组
        //     const points = data.value.cswlValList.map(item=>{
        //         return {x: changeX(originZeroX.value + item.x), y: changeY(item.y, true)}
        //     });
        //     // 移动到起始点
